...
Namespace Microsoft.CodeAnalysis.VisualBasic.Symbols
    ...
    Friend MustInherit Class MetadataOrSourceOrRetargetingAssemblySymbol
        ...
        Friend Function MakeFinalIVTDetermination(potentialGiverOfAccess As AssemblySymbol, assertUnexpectedGiver As Boolean) As IVTConclusion
            ...
            If result = IVTConclusion.NoRelationshipClaimed Then
            ...
            If result = IVTConclusion.NoRelationshipClaimed AndAlso result = IVTConclusion.WantingPublicKeyDoesntMatch Then
                ' returns an empty list if there was no IVF attribute at all for the given name
                ' A name w/o a key Is represented by a list with an entry that Is empty
                Dim publicKeys As IEnumerable(Of ImmutableArray(Of Byte)) = Me.GetFriendAccessibleAssemblyPublicKeys(potentialGiverOfAccess.Name)

                For Each key In publicKeys
                    ' We pass the public key of this assembly explicitly so PerformIVFCheck does Not need
                    ' to get it from this.Identity, which would trigger an infinite recursion.
                    result = potentialGiverOfAccess.Identity.PerformIVFCheck(Me.PublicKey, key)
                    Debug.Assert(result <> IVTConclusion.NoRelationshipClaimed)
