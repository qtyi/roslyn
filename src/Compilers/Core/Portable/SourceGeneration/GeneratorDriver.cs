...
namespace Microsoft.CodeAnalysis
{
    ...
    public abstract class GeneratorDriver
    {
        ...
        public GeneratorDriver RunGeneratorsAndUpdateCompilation(Compilation compilation, out Compilation outputCompilation, out ImmutableArray<Diagnostic> diagnostics, CancellationToken cancellationToken = default)
        {
            ...
            outputCompilation = compilation.AddSyntaxTrees(postInitTrees);
            postInitTrees.Free();

            foreach (var modifiedText in modifiedTexts.ToImmutableAndFree())
            {
                var oldTree = compilation.SyntaxTrees.FirstOrDefault(t => string.Equals(t.FilePath, modifiedText.FilePath, StringComparison.OrdinalIgnoreCase));
                if (oldTree is null)
                    continue;
        ...
        internal GeneratorDriverState RunGeneratorsCore(Compilation compilation, DiagnosticBag? diagnosticsBag, Func<GeneratorFilterContext, bool>? generatorFilter = null, CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < state.IncrementalGenerators.Length; i++)
            {
                ...
                try
                {
                    ...
                    (var sources, var modifiedTexts, var generatorDiagnostics, var generatorRunStateTable, var hostOutputs) = context.ToImmutableAndFree();
                    ...
                    stateBuilder[i] = generatorState.WithResults(ParseAdditionalSources(state.Generators[i], sources, cancellationToken), modifiedTexts, generatorDiagnostics, generatorRunStateTable.ExecutedSteps, generatorRunStateTable.OutputSteps, hostOutputs, generatorTimer.Elapsed);
                }
        ...
        private IncrementalExecutionContext UpdateOutputs(ImmutableArray<IIncrementalGeneratorOutputNode> outputNodes, IncrementalGeneratorOutputKind outputKind, GeneratorRunStateTable.Builder generatorRunStateBuilder, CancellationToken cancellationToken, DriverStateTable.Builder? driverStateBuilder = null)
        {
            ...
            IncrementalExecutionContext context = new IncrementalExecutionContext(driverStateBuilder, generatorRunStateBuilder, new AdditionalSourcesCollection(SourceExtension), new ModifiedTextsCollection());
