    ...
    internal sealed class LambdaSymbol : SourceMethodSymbolWithAttributes
    {
            ...
            ImmutableArray<RefKind> parameterRefKinds)
        {
            ...
            for (int p = 0; p < unboundLambda.ParameterCount; ++p)
            {
                ...
                var parameter = new LambdaParameterSymbol(owner: this, paramSyntax?.GetReference(), attributeLists, type, typeWithoutUnwrappingAliasTarget, ordinal: p, refKind, scope, name, unboundLambda.ParameterIsDiscard(p), isParams, location);
