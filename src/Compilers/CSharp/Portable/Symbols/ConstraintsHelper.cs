...
#nullable disable
        ...
        {
            CheckAliasConstructAnnotations(type, args);
            ...
            HashSet<TypeParameterSymbol> ignoreTypeConstraintsDependentOnTypeParametersOpt = null)
        {
            ...
            if (typeParameters.Length > 0 && substitution is not null)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    var aliasConstructAnnotationsCheckResult = CheckAliasConstructAnnotations(typeArguments[i].Type, in args);
                    if (aliasConstructAnnotationsCheckResult == AliasConstructAnnotation.NotSatisfied ||
                        !CheckConstraints(constructedContainingSymbol, in args, substitution, typeParameters[i], typeArguments[i], diagnosticsBuilder, nullabilityDiagnosticsBuilderOpt,
                                          ref useSiteDiagnosticsBuilder,
