...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests
{
    public partial class GreenNodeTests
    {
        ...
        private static Syntax.InternalSyntax.UsingDirectiveSyntax GenerateUsingDirective()
            => InternalSyntaxFactory.UsingDirective(null, InternalSyntaxFactory.Token(SyntaxKind.UsingKeyword), null, null, null, null, null, GenerateIdentifierName(), null, InternalSyntaxFactory.Token(SyntaxKind.SemicolonToken));
    ...
    public partial class RedNodeTests
    {
        ...
        private static UsingDirectiveSyntax GenerateUsingDirective()
            => SyntaxFactory.UsingDirective(default(SyntaxToken), SyntaxFactory.Token(SyntaxKind.UsingKeyword), default(SyntaxToken), default(SyntaxToken), default(SyntaxToken), default(TypeParameterListSyntax), default(SyntaxToken), GenerateIdentifierName(), default(SyntaxList<TypeParameterConstraintClauseSyntax>), SyntaxFactory.Token(SyntaxKind.SemicolonToken));
        ...
        public void TestUsingDirectiveFactoryAndProperties()
        {
            ...
            Assert.Equal(default, node.ConstraintClauses);
            Assert.Equal(SyntaxKind.SemicolonToken, node.SemicolonToken.Kind());
            var newNode = node.WithGlobalKeyword(node.GlobalKeyword).WithUsingKeyword(node.UsingKeyword).WithStaticKeyword(node.StaticKeyword).WithUnsafeKeyword(node.UnsafeKeyword).WithIdentifier(node.Identifier).WithTypeParameterList(node.TypeParameterList).WithEqualsToken(node.EqualsToken).WithNamespaceOrType(node.NamespaceOrType).WithConstraintClauses(node.ConstraintClauses).WithSemicolonToken(node.SemicolonToken);
            Assert.Equal(node, newNode);
        }
